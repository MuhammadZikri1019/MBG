...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 23 Nov 2025 pada 16.50
...
-- Prosedur
--
CREATE PROCEDURE `sp_generate_laporan_keseluruhan` (IN `p_tipe_laporan` ENUM('harian','mingguan','bulanan'), IN `p_tanggal_mulai` DATE, IN `p_tanggal_akhir` DATE, IN `p_id_dapur` INT, IN `p_dibuat_oleh` INT, IN `p_dibuat_oleh_tipe` ENUM('super_admin','pengelola'))   BEGIN
...
CREATE PROCEDURE `sp_generate_laporan_produksi` (IN `p_tipe_laporan` ENUM('harian','mingguan','bulanan'), IN `p_tanggal_mulai` DATE, IN `p_tanggal_akhir` DATE, IN `p_id_dapur` INT, IN `p_dibuat_oleh` INT, IN `p_dibuat_oleh_tipe` ENUM('super_admin','pengelola'))   BEGIN
    DECLARE v_kode_laporan VARCHAR(50);
    DECLARE v_judul VARCHAR(255);
    DECLARE v_data_json LONGTEXT;
...
CREATE PROCEDURE `sp_kurangi_stok_produksi` (IN `p_id_produksi` INT)   BEGIN
    -- ══════════════════════════════════════
...
CREATE PROCEDURE `sp_tambah_stok_pembelanjaan` (IN `p_id_pembelanjaan` INT)   BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE v_id_bahan INT;
...
--
-- NOTE: Triggers are commented out because Wasmer does not provide SUPER privilege
-- These triggers need to be handled in application code instead
/*
DELIMITER $$
